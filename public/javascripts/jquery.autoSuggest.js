...
(function($){
	$.fn.autoSuggest = function(data, options) {
        ...
        values_input.attr("name", input.attr("name"));
              ...
              results_holder.css("left", selections_holder.offset().left);
							results_holder.show();
						}
					}
					input_focus = true;
				...
				function processData(data, query){
					...
					var escape = function(str) { return str.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, "\\$&"); };
					query = escape(query);
					...
					for(var i=0;i<d_count;i++){
						...
						if(str){
							...
							if(str.search(escape(query)) != -1 && values_input.val().search(pat) == -1){
								forward = true;
							}
						}
						if(forward){
