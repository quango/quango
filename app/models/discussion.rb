class Discussion
  ...
  def self.related_discussions(discussion, opts = {})
    opts[:per_page] ||= 10
    opts[:page]     ||= 1
    opts[:group_id] = discussion.group_id
    opts[:banned] = false

    Discussion.paginate(opts.merge(:_keywords => {:$in => discussion.tags}, :_id => {:$ne => discussion.id}))
  end
