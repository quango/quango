class HeadersController < ApplicationController
  ...
  def desat
    @group = current_group
    @header = Header.find(params[:id])
    @header.header = @header.header.process!(:desat)
    @header.save

    respond_to do |format|
      format.html { redirect_to(crop_header_path, :notice => 'Header was desaturated.') }
    end
  ...
  def negate
    @group = current_group
    @header = Header.find(params[:id])
    @header.header = @header.header.process!(:negate)
    @header.save

    respond_to do |format|
      format.html { redirect_to(crop_header_path, :notice => 'Header was negated.') }
    end
  ...
  def watermark
    ...
    respond_to do |format|
      format.html { redirect_to(crop_header_path, :notice => 'Header was watermarked.') }
    end
  ...
  def flip
    @group = current_group
    @header = Header.find(params[:id])
    @header.header = @header.header.process!(:flip)
    @header.save

    respond_to do |format|
      format.html { redirect_to(crop_header_path, :notice => 'Header was successfully flipped so it claims.') }
    end
  ...
  def flop
    @group = current_group
    @header = Header.find(params[:id])
    @header.header = @header.header.process!(:flop)
    @header.save
  ...
  def rotate_180
    @group = current_group
    @header = Header.find(params[:id])
    @header.header = @header.header.process(:rotate, 180, :background_colour => 'white')
    @header.save
    respond_to do |format|
      format.html { redirect_to(crop_header_path, :notice => 'Header was successfully flopped so it claims.') }
    end
