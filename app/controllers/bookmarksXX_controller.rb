...
# Basically what we are trying to do here is grab all the data available, mung it together
# and return a set of sane keywords, people, locations and organisations
# to achieve this we only want to make two api calls

 def initialize(link)
  scrape =  Nokogiri::HTML(open(link, "User-Agent" => "Ruby/#{RUBY_VERSION}"))
  @rawtitle = scrape.xpath('//title')
  @rawkeywords = scrape.xpath("//meta[@name='keywords']/@content") #.to_s.gsub(/ and /,",").strip
  @rawdescription = scrape.xpath("//meta[@name='description']/@content")
  @raw = @rawtitle #+ @rawkeywords + @rawdescription
 end
...
class GetMetadata
 ...
 def metakeywords
  ...
  metakeywordss = @rawmetakeywords.to_s.gsub(/ and /,",")
  
 @metakeywords = metakeywordss
...
class URLGetRankedConcepts
 ...
 def initialize(link)
  ...
  parselink = endpoint << link << options
  rankedconceptsxml = Nokogiri::XML(open(parselink, "User-Agent" => "Ruby/#{RUBY_VERSION}"))
  extractconcepts = rankedconceptsxml.xpath("//keyword/text") 
  @parsedlink = parselink

  #let's quickly turn the hash into an array
  @ee = extractconcepts.to_s.gsub(/<text>/,"")
  @aa = @ee.to_s.gsub(/<\/text>/,",")


 end

 def extractedconcepts
   return @aa
 end
...
class URLGetRankedKeywords
 ...
 def initialize(link)
  mykey = AppConfig.alchemy["key"]
  endpoint = 'http://access.alchemyapi.com/calls/url/URLGetRankedKeywords?apikey='+mykey+'&url='	
  options = '&keywordExtractMode=normal&sourceText=cquery&cquery=p&maxRetrieve=24'
  parselink = endpoint << link << options
...
class URLGetNamedEntities
 ...
 def initialize(link)
  ...
  options = ''#&keywordExtractMode=strict&sourceText=cquery&cquery=p' #&maxRetrieve=24
  parselink = endpoint << link << options
  namedentitiesxml = Nokogiri::XML(open(parselink, "User-Agent" => "Ruby/#{RUBY_VERSION}"))
  extractedentities = namedentitiesxml.xpath("//entity/text") 
  @parsedlink = parselink

  #let's quickly turn the hash into an array
  @n = extractedentities.to_s.split(/<text>/)
  @ne = @n.to_s.split(/<\/text>/)
 ...
 def namedentities
  ...
  a.each do |k, v|
   puts "#{k} appears #{v} hgjs"
  end

  a.keys.sort_by {|key|-a[key]}.each do |key|
   if a[key]>0
    @namedentities << key
    @namedentities << '<sup>'+a[key].to_s+'</sup>'
    @namedentities << ','
   end
  end
...
class URLGetRankedEntities
 ...
 def entity
  ...
  @pop.each do |entity|
   if entity.xpath("/disambiguated/subType")
...
class BookmarksXXController < ApplicationController
 ...
 def showlink
  ...
  def create
   if @current_stage = "stage2"
    ...
    @question.safe_update(%w[title bookmark body language tags persons locations wiki], params[:question])
