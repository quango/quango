    ...
    if @page_title
      if current_group.name == AppConfig.application_name
        ...
        if current_group.isolate
          "#{@page_title} | #{current_group.name.capitalize} #{current_group.name_highlight} - #{current_group.legend}"
        else
          "#{@page_title} | #{current_group.name.capitalize} #{current_group.name_highlight} - #{current_group.legend}"
        end
      end
      ...
      if current_group.name == AppConfig.application_name
        "#{current_group.name.capitalize} #{current_group.name_highlight} | #{current_group.legend}"
      else
        if current_group.isolate
          "#{current_group.name.capitalize} #{current_group.name_highlight} | #{current_group.legend}"
        else
          "#{current_group.name.capitalize} #{current_group.name_highlight} | #{current_group.legend} - #{AppConfig.application_name}"
        end
      end
    end
  end
