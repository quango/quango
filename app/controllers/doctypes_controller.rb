class DoctypesController < ApplicationController
  ...
  def update
    ...
    @doctypes = current_group.doctypes
    @doctype = @doctypes.find_by_slug_or_id(params[:id])
    @doctype.safe_update(%w[name public_access top_dog custom_icon featured hidden slideshow has_video has_links product_format expanded create_label created_label help_text], params[:doctype])
