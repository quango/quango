...
module ApplicationHelper
  ...
  def item_topics(tags = [], options = {})
    ...
    tags.each do |tag|
      ...
      cloud << "<span>#{link_to(tag["name"], url, :class => "#{tag_class} #{css[size.round]}",:style => "background-color:#{current_group.secondary}")}</span>"
    end
    cloud += ''
    cloud
  end
  ...
  def tag_cloud(tags = [], options = {})
    ...
    tags.each do |tag|
      size = min_size + (tag["count"] - lowest_value["count"]) * ratio
      url = url_for(:controller => "items", :action => "index", :tags => tag["name"])
      cloud << "X<span>#{link_to(tag["name"], url, :class => "#{tag_class} #{css[size.round]}")}</span> "
    end
    cloud += "</div>"
    cloud
  end
  ...
  def item_section(item)
    ...
    @item_section = Doctype.find_by_id(@item.doctype_id)
    @item_section
  end
